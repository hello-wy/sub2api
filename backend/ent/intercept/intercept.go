...
// The DailyCheckinRecordFunc type is an adapter to allow the use of ordinary function as a Querier.
type DailyCheckinRecordFunc func(context.Context, *ent.DailyCheckinRecordQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f DailyCheckinRecordFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.DailyCheckinRecordQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.DailyCheckinRecordQuery", q)
}

// The TraverseDailyCheckinRecord type is an adapter to allow the use of ordinary function as Traverser.
type TraverseDailyCheckinRecord func(context.Context, *ent.DailyCheckinRecordQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseDailyCheckinRecord) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseDailyCheckinRecord) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.DailyCheckinRecordQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.DailyCheckinRecordQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.DailyCheckinRecordQuery:
		return &query[*ent.DailyCheckinRecordQuery, predicate.DailyCheckinRecord, dailycheckinrecord.OrderOption]{typ: ent.TypeDailyCheckinRecord, tq: q}, nil
