...
type Client struct {
	...
	// DailyCheckinRecord is the client for interacting with the DailyCheckinRecord builders.
	DailyCheckinRecord *DailyCheckinRecordClient
...
func (c *Client) init() {
	...
	c.DailyCheckinRecord = NewDailyCheckinRecordClient(c.config)
...
func (c *Client) Tx(ctx context.Context) (*Tx, error) {
	...
	return &Tx{
		...
		DailyCheckinRecord:            NewDailyCheckinRecordClient(cfg),
...
func (c *Client) Use(hooks ...Hook) {
	for _, n := range []interface{ Use(...Hook) }{
		...
		c.ChannelMonitorHistory, c.ChannelMonitorRequestTemplate, c.DailyCheckinRecord,
...
func (c *Client) Mutate(ctx context.Context, m Mutation) (Value, error) {
	...
	case *DailyCheckinRecordMutation:
		return c.DailyCheckinRecord.mutate(ctx, m)
...
// DailyCheckinRecordClient is a client for the DailyCheckinRecord schema.
type DailyCheckinRecordClient struct {
	config
}

// NewDailyCheckinRecordClient returns a client for the DailyCheckinRecord from the given config.
func NewDailyCheckinRecordClient(c config) *DailyCheckinRecordClient {
	return &DailyCheckinRecordClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `dailycheckinrecord.Hooks(f(g(h())))`.
func (c *DailyCheckinRecordClient) Use(hooks ...Hook) {
	c.hooks.DailyCheckinRecord = append(c.hooks.DailyCheckinRecord, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `dailycheckinrecord.Intercept(f(g(h())))`.
func (c *DailyCheckinRecordClient) Intercept(interceptors ...Interceptor) {
	c.inters.DailyCheckinRecord = append(c.inters.DailyCheckinRecord, interceptors...)
}

// Create returns a builder for creating a DailyCheckinRecord entity.
func (c *DailyCheckinRecordClient) Create() *DailyCheckinRecordCreate {
	mutation := newDailyCheckinRecordMutation(c.config, OpCreate)
	return &DailyCheckinRecordCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of DailyCheckinRecord entities.
func (c *DailyCheckinRecordClient) CreateBulk(builders ...*DailyCheckinRecordCreate) *DailyCheckinRecordCreateBulk {
	return &DailyCheckinRecordCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *DailyCheckinRecordClient) MapCreateBulk(slice any, setFunc func(*DailyCheckinRecordCreate, int)) *DailyCheckinRecordCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &DailyCheckinRecordCreateBulk{err: fmt.Errorf("calling to DailyCheckinRecordClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*DailyCheckinRecordCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for DailyCheckinRecord.
func (c *DailyCheckinRecordClient) Update() *DailyCheckinRecordUpdate {
	mutation := newDailyCheckinRecordMutation(c.config, OpUpdate)
	return &DailyCheckinRecordUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *DailyCheckinRecordClient) UpdateOne(_m *DailyCheckinRecord) *DailyCheckinRecordUpdateOne {
	mutation := newDailyCheckinRecordMutation(c.config, OpUpdateOne, withDailyCheckinRecord(_m))
	return &DailyCheckinRecordUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *DailyCheckinRecordClient) UpdateOneID(id int64) *DailyCheckinRecordUpdateOne {
	mutation := newDailyCheckinRecordMutation(c.config, OpUpdateOne, withDailyCheckinRecordID(id))
	return &DailyCheckinRecordUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for DailyCheckinRecord.
func (c *DailyCheckinRecordClient) Delete() *DailyCheckinRecordDelete {
	mutation := newDailyCheckinRecordMutation(c.config, OpDelete)
	return &DailyCheckinRecordDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *DailyCheckinRecordClient) DeleteOne(_m *DailyCheckinRecord) *DailyCheckinRecordDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *DailyCheckinRecordClient) DeleteOneID(id int64) *DailyCheckinRecordDeleteOne {
	builder := c.Delete().Where(dailycheckinrecord.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &DailyCheckinRecordDeleteOne{builder}
}

// Query returns a query builder for DailyCheckinRecord.
func (c *DailyCheckinRecordClient) Query() *DailyCheckinRecordQuery {
	return &DailyCheckinRecordQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeDailyCheckinRecord},
		inters: c.Interceptors(),
	}
}

// Get returns a DailyCheckinRecord entity by its id.
func (c *DailyCheckinRecordClient) Get(ctx context.Context, id int64) (*DailyCheckinRecord, error) {
	return c.Query().Where(dailycheckinrecord.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *DailyCheckinRecordClient) GetX(ctx context.Context, id int64) *DailyCheckinRecord {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// QueryUser queries the user edge of a DailyCheckinRecord.
func (c *DailyCheckinRecordClient) QueryUser(_m *DailyCheckinRecord) *UserQuery {
	query := (&UserClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(dailycheckinrecord.Table, dailycheckinrecord.FieldID, id),
			sqlgraph.To(user.Table, user.FieldID),
...
// Hooks returns the client hooks.
func (c *DailyCheckinRecordClient) Hooks() []Hook {
	return c.hooks.DailyCheckinRecord
}

// Interceptors returns the client interceptors.
func (c *DailyCheckinRecordClient) Interceptors() []Interceptor {
	return c.inters.DailyCheckinRecord
}

func (c *DailyCheckinRecordClient) mutate(ctx context.Context, m *DailyCheckinRecordMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&DailyCheckinRecordCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&DailyCheckinRecordUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&DailyCheckinRecordUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&DailyCheckinRecordDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown DailyCheckinRecord mutation op: %q", m.Op())
	}
}
...
// QueryDailyCheckinRecords queries the daily_checkin_records edge of a User.
func (c *UserClient) QueryDailyCheckinRecords(_m *User) *DailyCheckinRecordQuery {
	query := (&DailyCheckinRecordClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(user.Table, user.FieldID, id),
...
type (
	hooks struct {
		...
		ChannelMonitorRequestTemplate, DailyCheckinRecord, ErrorPassthroughRule, Group,
		IdempotencyRecord, IdentityAdoptionDecision, PaymentAuditLog, PaymentOrder,
		PaymentProviderInstance, PendingAuthSession, PromoCode, PromoCodeUsage, Proxy,
		RedeemCode, SecuritySecret, Setting, SubscriptionPlan, TLSFingerprintProfile,
		UsageCleanupTask, UsageLog, User, UserAllowedGroup, UserAttributeDefinition,
		UserAttributeValue, UserPlatformQuota, UserSubscription, WelfareRecord []ent.Hook
	}
	inters struct {
		APIKey, Account, AccountGroup, Announcement, AnnouncementRead, AuthIdentity,
		AuthIdentityChannel, BatchImageEvent, BatchImageItem, BatchImageJob,
		...
		RedeemCode, SecuritySecret, Setting, SubscriptionPlan, TLSFingerprintProfile,
		UsageCleanupTask, UsageLog, User, UserAllowedGroup, UserAttributeDefinition,
		UserAttributeValue, UserPlatformQuota, UserSubscription, WelfareRecord []ent.Interceptor
	}
)
