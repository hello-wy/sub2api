...
// HasWelfareRecords applies the HasEdge predicate on the "welfare_records" edge.
func HasWelfareRecords() predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
// HasWelfareRecordsWith applies the HasEdge predicate on the "welfare_records" edge with a given conditions (other predicates).
func HasWelfareRecordsWith(preds ...predicate.WelfareRecord) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := newWelfareRecordsStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
