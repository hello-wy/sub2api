...
func RegisterUserRoutes(
	...
	authenticated.Use(gin.HandlerFunc(auditLog))
	{
		...
		user := authenticated.Group("/user")
		{
			...
			user.GET("/attributes", h.User.GetAttributes)
			user.GET("/dashboard/users-ranking", h.Admin.Dashboard.GetUserSpendingRanking)
			user.PUT("/password", h.User.ChangePassword)
			user.PUT("", h.User.UpdateProfile)
			user.GET("/aff", h.User.GetAffiliate)
			user.GET("/checkin/status", h.User.GetCheckInStatus)
			user.GET("/checkin/history", h.User.ListCheckInHistory)
			user.POST("/aff/transfer", h.User.TransferAffiliateQuota)
			user.POST("/checkin", h.User.CheckInDaily)
