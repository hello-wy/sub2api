...
func ProvideHTTPServer(cfg *config.Config, router *gin.Engine) *http.Server {
	...
	if cfg.Server.H2C.Enabled {
		...
		log.Printf("HTTP/2 Cleartext (h2c) enabled: max_concurrent_streams=%d, idle_timeout=%ds, max_read_frame_size=%d, max_receive_buffer_per_connection=%d, max_receive_buffer_per_stream=%d",
