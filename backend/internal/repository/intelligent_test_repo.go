...
const intelligentRecordColumns = `t.id,t.account_id,t.test_type,t.status,t.score,t.result,t.result_image,t.input,t.raw_response,t.raw_truncated,t.error_message,t.duration_ms,t.model,t.anti_degradation,t.config_snapshot,t.evaluation,t.started_at,t.finished_at,t.created_at,COALESCE(t.lease_token,''),t.queue_reason,t.available_at,t.scheduled`
const intelligentSummaryColumns = `t.id,t.account_id,t.test_type,t.status,t.score,LEFT(t.result,1200),t.result_image,'','',t.raw_truncated,t.error_message,t.duration_ms,t.model,t.anti_degradation,'{}'::jsonb,t.evaluation,t.started_at,t.finished_at,t.created_at,'',t.queue_reason,t.available_at,t.scheduled`
...
func scanIntelligentRecord(row intelligentScanner) (*service.IntelligentTestRecord, error) {
	...
	var cfg, eval []byte
	err := row.Scan(&r.ID, &r.AccountID, &r.TestType, &r.Status, &r.Score, &r.Result, &r.ResultImage, &r.Input, &r.RawResponse, &r.RawTruncated, &r.ErrorMessage, &r.DurationMS, &r.Model, &r.AntiDegradation, &cfg, &eval, &r.StartedAt, &r.FinishedAt, &r.CreatedAt, &r.LeaseToken, &r.QueueReason, &r.AvailableAt, &r.Scheduled)
...
func (r *intelligentTestRepository) IsAdmin(ctx context.Context, id int64) (bool, error) {
	var ok bool
	err := r.db.QueryRowContext(ctx, `SELECT EXISTS(SELECT 1 FROM users WHERE id=$1 AND role IN ('admin','super_admin') AND status='active' AND deleted_at IS NULL)`, id).Scan(&ok)
	return ok, err
}
func (r *intelligentTestRepository) Settings(ctx context.Context) ([]service.IntelligentTestSetting, error) {
	rows, err := r.db.QueryContext(ctx, `SELECT s.test_type,s.enabled,s.user_visible,s.config,s.updated_at,
COALESCE(sc.enabled,false),COALESCE(sc.interval_minutes,60),COALESCE(sc.account_ids,'{}'::bigint[]),sc.next_run_at,sc.last_run_at,COALESCE(sc.last_error,'')
FROM test_settings s LEFT JOIN intelligent_test_schedules sc ON sc.test_type=s.test_type ORDER BY s.test_type`)
	if err != nil {
		return nil, err
	}
	defer func() { _ = rows.Close() }()
	...
	for rows.Next() {
		...
		var cfg []byte
		s.Schedule = &service.IntelligentTestSchedule{AccountIDs: []int64{}}
		if err := rows.Scan(&s.TestType, &s.Enabled, &s.UserVisible, &cfg, &s.UpdatedAt, &s.Schedule.Enabled, &s.Schedule.IntervalMinutes, pq.Array(&s.Schedule.AccountIDs), &s.Schedule.NextRunAt, &s.Schedule.LastRunAt, &s.Schedule.LastError); err != nil {
			return nil, err
		}
		if err := json.Unmarshal(cfg, &s.Config); err != nil {
			return nil, err
		}
		service.NormalizeIntelligentTestConfigForType(s.TestType, &s.Config)
		s.Name = s.TestType
		switch s.TestType {
		case "pelican":
			s.Name = "鹈鹕测试"
...
func (r *intelligentTestRepository) UpdateSetting(ctx context.Context, actor int64, s *service.IntelligentTestSetting) error {
	...
	err = tx.QueryRowContext(ctx, `SELECT jsonb_build_object('enabled',s.enabled,'user_visible',s.user_visible,'config',s.config,'schedule',(SELECT to_jsonb(sc) - 'requested_by' - 'test_type' - 'updated_at' FROM intelligent_test_schedules sc WHERE sc.test_type=s.test_type)) FROM test_settings s WHERE s.test_type=$1 FOR UPDATE`, s.TestType).Scan(&previous)
	if errors.Is(err, sql.ErrNoRows) {
	...
	err = tx.QueryRowContext(ctx, `UPDATE test_settings SET enabled=$2,user_visible=$3,config=$4,updated_at=NOW() WHERE test_type=$1 RETURNING updated_at`, s.TestType, s.Enabled, s.UserVisible, string(cfg)).Scan(&s.UpdatedAt)
	if err != nil {
		return err
	}
	if err = updateIntelligentSchedule(ctx, tx, actor, s); err != nil {
		return err
	}
...
SELECT (SELECT COUNT(DISTINCT t.account_id) FROM account_tests t JOIN selected a ON a.id=t.account_id WHERE t.finished_at>=date_trunc('day',NOW()) AND t.status<>'cancelled' AND ` + typeCondition + `),COUNT(*) FILTER(WHERE success AND NOT abnormal AND NOT review),COUNT(*) FILTER(WHERE abnormal),COUNT(*) FILTER(WHERE review) FROM per_account`
	...
	if f.TestType == "" || f.TestType == "pelican" {
		if err := r.prepareAdminPelicanPreviews(ctx, ids); err != nil {
			return nil, err
		}
	}
	...
	completedRows, err := r.db.QueryContext(ctx, `SELECT `+intelligentSummaryColumns+` FROM account_tests t JOIN (SELECT DISTINCT ON(t.account_id,t.test_type) t.id FROM account_tests t WHERE t.account_id=ANY($1) AND `+publicIntelligentResultSQL+` ORDER BY t.account_id,t.test_type,t.id DESC) latest ON latest.id=t.id`, pq.Array(ids))
	if err != nil {
		return nil, err
	}
