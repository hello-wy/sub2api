...
func (r *pelicanShowcaseRepository) Publish(ctx context.Context, result *service.ScheduledTestResult, groupIDs []int64, maxItems int) error {
 ...
 (group_id, source_result_id, model_id, reasoning_effort, response_text, latency_ms, generated_at, source_scope)
 SELECT DISTINCT COALESCE(p.group_id, ag.group_id), $2::bigint, $3::text, $4::text, $5::text, $6::bigint, $7::timestamptz, CASE WHEN p.group_id IS NULL THEN 'account' ELSE 'group' END
 FROM scheduled_test_plans p
 LEFT JOIN accounts a ON a.id = p.account_id AND a.deleted_at IS NULL
 LEFT JOIN account_groups ag ON ag.account_id = a.id AND p.group_id IS NULL
 WHERE p.id = $1 AND COALESCE(p.group_id, ag.group_id) = ANY($8)
...
func (r *pelicanShowcaseRepository) ListItems(ctx context.Context, groupIDs []int64, maxItems int, since time.Time) ([]*service.PelicanShowcaseItem, error) {
	rows, err := r.db.QueryContext(ctx, `SELECT id, group_id, model_id, reasoning_effort, latency_ms, generated_at, source_scope FROM (
 SELECT id, group_id, model_id, reasoning_effort, latency_ms, generated_at, source_scope,
  ROW_NUMBER() OVER (PARTITION BY group_id ORDER BY generated_at DESC, id DESC) AS rn
 FROM pelican_showcase_items WHERE group_id = ANY($1)
) ranked
...
ORDER BY group_id, generated_at DESC, id DESC`, pq.Array(groupIDs), maxItems, nullableTime(since))
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.GroupID, &item.ModelID, &item.ReasoningEffort, &item.LatencyMs, &item.GeneratedAt, &item.SourceScope); err != nil {
			return nil, err
		}
...
func (r *pelicanShowcaseRepository) GetItem(ctx context.Context, id int64, groupIDs []int64, maxItems int, since time.Time) (*service.PelicanShowcaseItem, error) {
	item := &service.PelicanShowcaseItem{}
	err := r.db.QueryRowContext(ctx, `SELECT i.id, i.group_id, i.model_id, i.reasoning_effort, i.latency_ms, i.generated_at, i.response_text, i.source_scope
 ...
 AND (SELECT COUNT(*) FROM pelican_showcase_items newer WHERE newer.group_id = i.group_id
  AND (newer.generated_at, newer.id) > (i.generated_at, i.id)) < $3`,
		...
		&item.ID, &item.GroupID, &item.ModelID, &item.ReasoningEffort, &item.LatencyMs, &item.GeneratedAt, &item.ResponseText, &item.SourceScope)
