...
func TestGroupDetailStatsHistoricalCostsAndCurrentBindings(t *testing.T) {
	...
	_, err := db.Exec(`CREATE TABLE account_ip_channels(account_id BIGINT PRIMARY KEY,logical_account_id BIGINT,retired_at TIMESTAMPTZ);
INSERT INTO accounts(id,name,platform,type,credentials,extra,created_at,updated_at) VALUES(1001,'logical-root','openai','oauth','{}','{}',NOW(),NOW()),(1002,'channel-2','openai','oauth','{}','{}',NOW(),NOW()),(1003,'channel-3','openai','oauth','{}','{}',NOW(),NOW());
INSERT INTO account_groups(account_id,group_id,created_at) VALUES(1001,900,NOW()),(1002,900,NOW()),(1003,900,NOW());
INSERT INTO account_ip_channels VALUES(1001,1001,NOW()),(1002,1001,NULL),(1003,1001,NULL);
CREATE TABLE api_keys(id BIGINT PRIMARY KEY,group_id BIGINT,status TEXT,expires_at TIMESTAMPTZ,deleted_at TIMESTAMPTZ);
...
(4,901,99,99,0,0,999,999,999,1,0,9999,'2026-09-10T00:00:00Z');`)
	...
	require.EqualValues(t, 1, stats.TotalAccounts, "live channels of one logical account count once even with a retired root")
