...
(SELECT COUNT(DISTINCT COALESCE(c.logical_account_id,a.id)) FROM account_groups ag JOIN accounts a ON a.id=ag.account_id LEFT JOIN account_ip_channels c ON c.account_id=a.id WHERE ag.group_id=g.id AND a.deleted_at IS NULL AND c.retired_at IS NULL),
