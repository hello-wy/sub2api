...
// Schedules and manual runs use one transactional enqueue path so capacity,
// idempotency and active-job conflicts remain consistent across replicas.
func (r *intelligentTestRepository) enqueueIntelligentTestsTx(ctx context.Context, tx *sql.Tx, actor int64, req service.IntelligentTestEnqueue) (*service.IntelligentTestEnqueued, error) {
	...
	payload, _ := json.Marshal(struct {
		...
		ReasoningEfforts map[string]string `json:",omitempty"`
	}{accountIDs, types, req.Models, req.ReasoningEfforts})
	...
	if err == nil {
		...
		return &service.IntelligentTestEnqueued{Records: records, Reused: true, ReusedCount: len(records)}, nil
	}
	...
	for _, kind := range types {
		...
		service.NormalizeIntelligentTestConfigForType(kind, &cfg)
		settings[kind] = cfg
	}
	...
	for _, id := range accountIDs {
		...
		for _, kind := range types {
			...
			// A present empty entry explicitly requests the upstream default;
			// an absent entry inherits the saved setting.
			if effort, exists := req.ReasoningEfforts[kind]; exists {
				cfg.ReasoningEffort = effort
			}
			if err := service.ValidateIntelligentReasoningEffort(cfg.ReasoningEffort); err != nil {
				return nil, err
			}
			...
			if activeErr == nil {
				var activeConfig service.IntelligentTestConfig
				if active.ConfigSnapshot != nil {
					activeConfig = *active.ConfigSnapshot
					service.NormalizeIntelligentTestConfigForType(kind, &activeConfig)
				}
				if active.ConfigSnapshot == nil || !service.SameIntelligentTestConfig(activeConfig, cfg) {
