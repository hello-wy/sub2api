...
func intelligentTestDB(t *testing.T) *sql.DB {
 ...
 CREATE TABLE accounts(id BIGINT PRIMARY KEY,name TEXT NOT NULL DEFAULT '',notes TEXT,platform TEXT NOT NULL DEFAULT 'openai',type TEXT NOT NULL DEFAULT 'apikey',status TEXT NOT NULL DEFAULT 'active',extra JSONB NOT NULL DEFAULT '{}',deleted_at TIMESTAMPTZ,schedulable BOOLEAN NOT NULL DEFAULT true,auto_pause_on_expired BOOLEAN NOT NULL DEFAULT true,expires_at TIMESTAMPTZ,rate_limit_reset_at TIMESTAMPTZ,overload_until TIMESTAMPTZ,temp_unschedulable_until TIMESTAMPTZ);
 ...
 INSERT INTO user_allowed_groups VALUES(2,100),(3,101);`)
	...
	migration, err = os.ReadFile("../../migrations/249_intelligent_test_schedules.sql")
	require.NoError(t, err)
	_, err = db.Exec(string(migration))
	require.NoError(t, err)
	_, err = db.Exec(string(migration))
	require.NoError(t, err, "schedule migration must be safely repeatable")
	return db
}
