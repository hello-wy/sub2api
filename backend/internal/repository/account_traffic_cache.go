...
// Separate adaptive state for ON/OFF keeps 429 reductions from constraining
// observation-only mode. Request, RPM and upstream outcome counters are shared.
...
local recField=ARGV[11]=='1' and 'recommended_all' or 'recommended_5xx'
local recommended=tonumber(redis.call('HGET',KEYS[1],recField) or tostring(hard))
...
redis.call('HSET',KEYS[1],'revision',math.max(rev,oldrev),'signature',sig,recField,recommended,'tokens',tokens,'refilled_at',now)
...
func (c *accountTrafficCache) Acquire(ctx context.Context, p service.AccountTrafficPlan, id string) (service.AccountTrafficAdmission, error) {
	...
	values, err := trafficAcquireScript.Run(ctx, c.rdb, trafficKeys(p.AccountID), p.Revision, p.Signature, p.HardLimit, flag(v.StrictRPMEnabled), v.RPM, v.Burst, flag(v.AdaptiveEnabled), v.AdaptiveMode, v.MinConcurrency, id, flag(service.Context429Enforcement(ctx))).Int64Slice()
	if err != nil {
...
local enforce429=ARGV[11]=='1'
local function observe(suffix, include429)
local function field(name) return name..suffix end
local limit=math.max(minimum,math.min(hard,tonumber(redis.call('HGET',KEYS[1],field('recommended')) or tostring(hard))))
local bad=(include429 and status==429) or (status>=500 and status<600)
if bad then
 local begin=tonumber(redis.call('HGET',KEYS[1],field('failure_window')) or '0')
 local failures=tonumber(redis.call('HGET',KEYS[1],field('failures')) or '0')
 if now-begin>=window then begin=now;failures=0 end
 failures=failures+1
 redis.call('HSET',KEYS[1],field('failure_window'),begin,field('failures'),failures,field('last_bad'),now,field('healthy_successes'),0)
 local changed=tonumber(redis.call('HGET',KEYS[1],field('last_adjustment')) or '0')
 if failures>=threshold and now-changed>=10000 then
  local reduced=math.max(minimum,math.floor(limit/2))
  if reduced<limit then redis.call('HSET',KEYS[1],field('recommended'),reduced,field('last_adjustment'),now,field('failures'),0) end
 end
elseif status>=200 and status<300 then
 local good=redis.call('HINCRBY',KEYS[1],field('healthy_successes'),1)
 local badAt=tonumber(redis.call('HGET',KEYS[1],field('last_bad')) or redis.call('HGET',KEYS[1],'observed_since') or tostring(now))
 local changed=tonumber(redis.call('HGET',KEYS[1],field('last_adjustment')) or '0')
 if good>=3 and now-badAt>=recovery and now-changed>=recovery and limit<hard then
  redis.call('HSET',KEYS[1],field('recommended'),limit+1,field('last_adjustment'),now,field('healthy_successes'),0)
 end
end
end
-- Both histories continue tracking 5xx/recovery while the switch changes.
-- Only the ON history accepts 429 as a reason to lower its limit.
observe('_5xx',false)
observe('_all',enforce429)
...
func (c *accountTrafficCache) Finish(ctx context.Context, p service.AccountTrafficPlan, id string, status int, duration int64) error {
	...
	return trafficFinishScript.Run(ctx, c.rdb, trafficKeys(p.AccountID)[:2], id, status, duration, p.Signature, enabled, p.HardLimit, v.MinConcurrency, v.FailureThreshold, v.FailureWindowSeconds, v.RecoverySeconds, boolToInt429(service.Context429Enforcement(ctx))).Err()
}
...
func (c *accountTrafficCache) Snapshot(ctx context.Context, p service.AccountTrafficPlan) (service.AccountTrafficState, error) {
	...
	state := service.AccountTrafficState{EffectiveConcurrency: p.HardLimit, RecommendedConcurrency: int(number(adaptiveField429(ctx, "recommended"))), Accepted: number("accepted"), RejectedRPM: number("rejected_rpm"), RejectedConcurrency: number("rejected_concurrency"), Upstream429: number("upstream_429"), Upstream5xx: number("upstream_5xx"), Completed: number("completed")}
	...
	if raw := values[adaptiveField429(ctx, "last_adjustment")]; raw != "" {
...
func boolToInt429(value bool) int {
	if value {
		return 1
	}
	return 0
}
func adaptiveField429(ctx context.Context, field string) string {
