...
func preserveLockedAccountProtection(ctx context.Context, client *dbent.Client, a *service.Account) error {
	...
	// Quarantine is server-owned and independent from editable protection
	// settings. Preserve the committed marker under the same row lock.
	delete(a.Extra, service.AccountModelMismatchExtraKey)
	if current.HasModelMismatch() {
		if a.Extra == nil {
			a.Extra = map[string]any{}
		}
		a.Extra[service.AccountModelMismatchExtraKey] = extra[service.AccountModelMismatchExtraKey]
		if current.IsModelMismatchQuarantined() {
			a.Schedulable = false
		}
	}
