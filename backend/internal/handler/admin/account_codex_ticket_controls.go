...
func (h *AccountHandler) accountTicketSummary(ctx context.Context, account *service.Account) *service.CodexAccountTicketStatus {
	if renderer, ok := h.codexAccountTickets.(interface {
		CodexAccountTicketStatusFromAccount(context.Context, *service.Account) *service.CodexAccountTicketStatus
	}); ok {
		return renderer.CodexAccountTicketStatusFromAccount(ctx, account)
	}
	return nil
}
