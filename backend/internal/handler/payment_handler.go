...
func (h *PaymentHandler) GetPlans(c *gin.Context) {
	...
	type planWithPlatform struct {
		...
		SubscriptionQuotaResetMode string   `json:"subscription_quota_reset_mode"`
	...
	for _, p := range plans {
		...
		result = append(result, planWithPlatform{
			...
			SubscriptionQuotaResetMode: gi.SubscriptionQuotaResetMode,
...
func (h *PaymentHandler) GetCheckoutInfo(c *gin.Context) {
	...
	loyalty, err := h.paymentService.GetLoyaltyInfo(ctx, subject.UserID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
type checkoutInfoResponse struct {
	...
	Loyalty                       *service.PaymentLoyaltyInfo     `json:"loyalty,omitempty"`
}
