...
func TestSettingService_UpdateSettings_PersistsWelfareAndLoyaltySettings(t *testing.T) {
	...
	require.Equal(t, "0.001", repo.updates[SettingKeyDailyCheckinRewardMin])
	require.Equal(t, "3", repo.updates[SettingKeyDailyCheckinRewardMax])
	require.Equal(t, `[{"min":0.001,"max":1,"probability":0.5},{"min":1,"max":3,"probability":0.5}]`, repo.updates[SettingKeyDailyCheckinRewardRanges])
