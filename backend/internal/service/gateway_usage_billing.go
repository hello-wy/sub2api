...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput) error {
	quarantineForwardResultModelMismatch(ctx, s.accountRepo, input.Account, input.Result)
	result := input.Result
	apiKey := input.APIKey
	user := input.User
	account := input.Account
	...
	if responseModel := responseModelBillingDeclaration(
		...
		if identified, responseChannelPriced := s.hasIdentifiedResponseModelPricing(ctx, responseModel, apiKey); identified {
			responseCost, err := s.calculateRecordUsageCost(ctx, result, apiKey, responseModel, multiplier, imageMultiplier, pricingAt, &recordUsageOpts{})
			if err != nil {
