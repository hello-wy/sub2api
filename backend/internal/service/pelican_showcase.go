...
type PelicanShowcaseRepository interface {
	// Publish copies group results only to their entry group; account results go to listed memberships,
...
// Only drawing questions with real HTML/SVG output qualify: answer-only candy
// questions share the plan type. Errors are logged, never returned, so
...
func (s *PelicanShowcaseService) PublishScheduledResult(ctx context.Context, result *ScheduledTestResult) {
	...
	if cfg := result.PelicanConfig; cfg.QuestionKind == "candy" || isBuiltinCandyPlan(cfg) {
		return
	}
