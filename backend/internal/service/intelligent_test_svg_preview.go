...
var (
	...
	svgLength                = regexp.MustCompile(`^([+-]?(?:[0-9]+(?:\.[0-9]*)?|\.[0-9]+)(?:[eE][+-]?[0-9]+)?)(?:%|px|em|ex|ch|rem|vw|vh|vmin|vmax|cm|mm|Q|in|pt|pc)?$`)
	svgPathNumber            = regexp.MustCompile(`^[+-]?(?:[0-9]+(?:\.[0-9]*)?|\.[0-9]+)(?:[eE][+-]?[0-9]+)?`)
)

// Bump when previously unavailable static drawings can be rendered. Historical
// negative cache entries are retried once per version, within the read budget.
const IntelligentSVGPreviewVersion = 2
...
func svgBasicDrawable(name string, values map[string]string) bool {
	positive := func(key string) bool {
		length := svgLength.FindStringSubmatch(strings.TrimSpace(values[key]))
		if length == nil {
			return false
		}
		value, err := strconv.ParseFloat(length[1], 64)
	...
	case "path":
		return svgPathHasDrawing(values["d"])
	case "line":
...
// A moveto's remaining coordinate pairs are implicit lineto segments. Parse
// complete parameter groups rather than requiring a literal drawing command.
// This is a bounded presence check; the sanitized SVG still renders as SVG.
func svgPathHasDrawing(path string) bool {
	var command byte
	first, drawing := true, false
	for {
		path = strings.TrimLeft(path, " \t\r\n,")
		if path == "" {
			return drawing
		}
		if strings.ContainsRune("MmLlHhVvCcSsQqTtAaZz", rune(path[0])) {
			command, path = path[0], path[1:]
		...
		for i := 0; i < arity; i++ {
			path = strings.TrimLeft(path, " \t\r\n,")
			// Arc flags are single digits and may directly precede another
			// flag or endpoint coordinate (e.g. A10 10 0 0110 20).
			if (command == 'A' || command == 'a') && (i == 3 || i == 4) {
				if path == "" || (path[0] != '0' && path[0] != '1') {
					return drawing
				}
				path = path[1:]
				continue
			}
			number := svgPathNumber.FindString(path)
			if number == "" {
				return drawing
			}
...
func evaluateIntelligentSVG(output string) IntelligentTestEvaluation {
	...
	detail["image_preview_version"] = IntelligentSVGPreviewVersion
