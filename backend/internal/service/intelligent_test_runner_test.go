...
func TestResolveIntelligentTestModelCodexFallback(t *testing.T) {
	...
	require.Equal(t, "gpt-5.6-sol", resolveIntelligentTestModel(codex, ""))
...
func TestResolveIntelligentTestModelProtocolDefaults(t *testing.T) {
	for _, tc := range []struct {
		platform, protocol, want string
	}{
		{PlatformOpenAI, "", "gpt-5.6-sol"},
		{PlatformGemini, "", "gemini-2.0-flash"},
		{PlatformAnthropic, "", "claude-sonnet-4-5-20250929"},
		{PlatformGrok, "", grokDefaultResponsesModel},
		{PlatformAntigravity, "", defaultAntigravityTestModel},
		{PlatformDeepseek, APIProtocolChatCompletions, "gpt-5.6-sol"},
		{PlatformDeepseek, APIProtocolAdaptive, "gpt-5.6-sol"},
		{PlatformDeepseek, APIProtocolResponses, "gpt-5.6-sol"},
		{PlatformDeepseek, APIProtocolAnthropic, "claude-sonnet-4-5-20250929"},
	} {
		t.Run(tc.platform+"/"+tc.protocol, func(t *testing.T) {
			a := &Account{Platform: tc.platform, Type: AccountTypeAPIKey, Credentials: map[string]any{"api_protocol": tc.protocol}}
			require.Equal(t, tc.want, resolveIntelligentTestModel(a, "  "))
			require.Equal(t, "explicit-model", resolveIntelligentTestModel(a, " explicit-model "))
		})
	}
}

func TestIntelligentRunnerDefaultModelRespectsCooldownAndMapping(t *testing.T) {
	for _, scheduled := range []bool{false, true} {
		for _, mapped := range []bool{false, true} {
			t.Run(fmt.Sprintf("scheduled=%v/mapped=%v", scheduled, mapped), func(t *testing.T) {
				calls := 0
				...
				repo.account.Status, repo.account.Schedulable = StatusActive, true
				actualModel := "gpt-5.6-sol"
				if mapped {
					actualModel = "mapped-default-model"
					repo.account.Credentials["model_mapping"] = map[string]any{"gpt-5.6-sol": actualModel}
				}
				until := time.Now().Add(90 * time.Second)
				setAccountModelRateLimitSnapshot(repo.account, actualModel, until, "cooldown", time.Now())
				r := &IntelligentTestRecord{AccountID: 81, Scheduled: scheduled, Input: "test", ConfigSnapshot: &IntelligentTestConfig{}}
				err := svc.RunIntelligentTest(context.Background(), r)
				var wait *TestAdmissionWaitError
				require.ErrorAs(t, err, &wait)
				require.WithinDuration(t, until, wait.Until, time.Second)
				require.Equal(t, "gpt-5.6-sol", r.Model)
				require.Empty(t, r.ConfigSnapshot.Model, "keep the user's default selection in the snapshot")
				require.Zero(t, calls)
				setAccountModelRateLimitSnapshot(repo.account, actualModel, time.Now().Add(-time.Second), "expired", time.Now())
				require.NoError(t, svc.RunIntelligentTest(context.Background(), r))
				require.Equal(t, 1, calls)
				require.Equal(t, actualModel, sentModel)
				require.Equal(t, actualModel, r.ConfigSnapshot.Execution.Model)
