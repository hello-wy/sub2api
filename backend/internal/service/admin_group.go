...
func (s *adminServiceImpl) CreateGroup(ctx context.Context, input *CreateGroupInput) (*Group, error) {
	...
	if err := validateFiniteLimits(input.SubscriptionTotalLimitUSD, input.DailyLimitUSD, input.WeeklyLimitUSD, input.MonthlyLimitUSD); err != nil {
		return nil, err
	}
	quotaResetMode, err := NormalizeSubscriptionQuotaResetMode(subscriptionType, input.SubscriptionQuotaResetMode)
	if err != nil {
		return nil, err
	}
	...
	group := &Group{
		...
		SubscriptionQuotaResetMode:      quotaResetMode,
...
func validateFiniteLimits(limits ...*float64) error {
	for _, limit := range limits {
		if limit != nil && (math.IsNaN(*limit) || math.IsInf(*limit, 0)) {
			return errors.New("limit must be finite")
		}
	}
	return nil
}
