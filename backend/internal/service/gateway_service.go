...
func (s *GatewayService) initDebugGatewayBodyFile(path string) {
	...
	if info, err := os.Stat(path); err == nil && info.IsDir() { //nolint:gosec // debug log path comes from trusted server-side environment configuration.
		path = filepath.Join(path, debugGatewayBodyDefaultFilename)
	}

	// 确保父目录存在
	if dir := filepath.Dir(path); dir != "." {
		if err := os.MkdirAll(dir, 0755); err != nil { //nolint:gosec // debug log path comes from trusted server-side environment configuration.
			slog.Error("failed to create gateway debug log directory", "dir", dir, "error", err)
			return
		}
	}

	//nolint:gosec // debug log path comes from trusted server-side environment configuration.
