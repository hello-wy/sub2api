<template>
  <BaseDialog :show="recordId !== null" :title="record ? `${testName(record.test_type)} · #${record.account_id}` : '测试详情'" width="wide" @close="emit('close')">
    ...
    <div v-else-if="record" class="space-y-5">
      <template v-if="record.test_type === 'pelican'">
        <p class="text-xs text-gray-500">{{ testTime(record.created_at) }} · {{ record.model || '上游默认' }}</p>
        <p v-if="isPending(record.status)" class="py-8 text-center text-sm text-gray-500">{{ record.status === 'queued' ? '正在排队生成图像…' : '正在生成图像…' }}</p>
        ...
        <details v-if="!publicView" class="rounded-xl border border-gray-200 p-4 dark:border-dark-700">
          <summary class="cursor-pointer text-sm text-gray-500">运行诊断</summary>
          <div class="mt-4 space-y-4 text-sm">
            <p>耗时：{{ (record.duration_ms / 1000).toFixed(1) }} 秒 · 检测时账号保护：{{ record.anti_degradation ? '开启' : '关闭' }}</p>
            <p v-if="record.queue_reason">{{ record.queue_reason }}</p>
            <p v-if="record.available_at && record.status === 'queued'">下次检查：{{ testTime(record.available_at) }}</p>
            <p v-if="record.error_message" class="text-amber-700">{{ record.error_message }}</p>
            <section v-if="record.input"><h4 class="mb-2 font-medium">测试输入</h4><pre class="test-output">{{ record.input }}</pre></section>
            <section><h4 class="mb-2 font-medium">模型原始答复 / SVG 源码</h4><pre class="test-output">{{ record.result || '暂无输出' }}</pre></section>
            <details v-if="record.raw_response"><summary class="cursor-pointer">原始响应{{ record.raw_truncated ? '（超过存储上限，已截断）' : '' }}</summary><pre class="test-output mt-2">{{ typeof record.raw_response === 'string' ? record.raw_response : JSON.stringify(record.raw_response, null, 2) }}</pre></details>
            <details v-if="record.config_snapshot"><summary class="cursor-pointer">本次测试配置</summary><pre class="test-output mt-2">{{ JSON.stringify(record.config_snapshot, null, 2) }}</pre></details>
          </div>
        </details>
      </template>
      <template v-else>
      ...
      <section><h4 class="mb-2 text-sm font-semibold">测试结果</h4><pre class="test-output">{{ record.result || '暂无输出' }}</pre></section>
    ...
    <template #footer>
      <div class="flex w-full flex-wrap justify-end gap-2">
        <button v-if="record && (!publicView || record.test_type !== 'pelican')" class="btn btn-secondary" @click="copy">{{ copied ? '已复制' : '复制测试信息' }}</button>
...
const canReevaluate = computed(() => {
  ...
  return !props.publicView && item && item.test_type !== 'pelican' && !item.raw_truncated && item.evaluation?.evaluator_version !== currentEvaluatorVersion && !!item.result &&
