<template>
  <article class="flex min-w-0 flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-dark-700 dark:bg-dark-800">
    ...
    <button class="m-4 mb-3 flex aspect-[4/3] items-center justify-center overflow-hidden rounded-xl bg-gray-50 text-left dark:bg-dark-900"
      ...
      <div v-else-if="summary.test_type !== 'pelican' && resultRecord?.result" class="max-h-full overflow-hidden p-5">
    ...
    <div class="flex items-center justify-between gap-2 px-4">
      <TestStatusBadge v-if="summary.test_type !== 'pelican'" :status="record?.status ?? 'waiting'" />
      <span v-else-if="isPending(record?.status)" class="text-xs text-gray-500">{{ record?.status === 'queued' ? '正在排队生成' : '正在生成新图像' }}</span>
    ...
    <TestAssessment v-if="resultRecord && summary.test_type !== 'pelican'" class="px-4 pt-3" :assessment="resultRecord.evaluation" :status="resultRecord.status" />
    ...
    <p v-if="summary.risk && summary.test_type !== 'pelican'" class="mx-4 mt-3 rounded-lg bg-amber-50 p-2 text-xs leading-relaxed text-amber-800 dark:bg-amber-900/20 dark:text-amber-300">⚠ {{ summary.risk }}</p>
    <details v-if="summary.test_type === 'pelican' && (record?.error_message || summary.risk)" class="mx-4 mt-3 text-xs text-gray-500"><summary class="cursor-pointer">运行诊断</summary><p class="mt-2">{{ record?.error_message || summary.risk }}</p></details>
