...
describe('admin UsageTable TPS', () => {
  it('formats generated token throughput and shows a placeholder when unavailable', () => {
    const wrapper = mount(UsageTable, {
      props: {
        data: [
    ...
    expect(wrapper.findAll('[data-testid="latency-tps"]').map((node) => node.text())).toEqual(['12.3 t/s', '-'])
  })
})
...
const DataTableStubWithLatency = {
  props: ['data'],
  template: '<div><div v-for="row in data" :key="row.request_id" :data-row="row.request_id"><slot name="cell-latency" :row="row" /></div></div>',
}
