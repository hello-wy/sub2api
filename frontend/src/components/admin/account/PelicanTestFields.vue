<template>
  <div class="space-y-3">
    <label v-if="!drawingOnly" class="input-label">{{ t('admin.accounts.pelicanTest.question') }}</label>
    <Select v-if="!drawingOnly" :model-value="modelValue.question_kind || 'pelican'" :options="questionOptions" @update:model-value="selectQuestion" />
    ...
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
      <div>
        <label class="input-label">{{ t('admin.accounts.pelicanTest.reasoning') }}</label>
...
const props = defineProps<{ modelValue: PelicanTestConfig; drawingOnly?: boolean }>()
