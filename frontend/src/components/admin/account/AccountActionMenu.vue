      ...
      >
        <div class="py-1">
          <template v-if="account">
            ...
            <button v-if="isOpenAIOAuthParent" @click="$emit('risk-control-check', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm text-amber-700 hover:bg-gray-100 dark:text-amber-400 dark:hover:bg-dark-700">
              <Icon name="exclamationTriangle" size="sm" />
              {{ t('admin.accounts.riskControl.checkAction') }}
            </button>
...
const emit = defineEmits(['close', 'test', 'stats', 'schedule', 'duplicate', 'reauth', 'refresh-token', 'recover-state', 'reset-quota', 'set-privacy', 'create-spark-shadow', 'risk-control-check'])
const { t } = useI18n()
