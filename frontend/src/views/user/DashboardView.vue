      ...
      <template v-else-if="stats">
        <header class="dashboard-header">
          <div>
            ...
            <h1>{{ greeting }}，{{ user?.username || user?.email || '朋友' }} <span>👋</span></h1>
        ...
        <section class="hero-metrics">
          ...
          <article class="hero-card"><span>今日 API 调用</span><strong>{{ formatNumber(stats.today_requests) }}</strong><small>全部请求 {{ formatNumber(stats.total_requests) }} 次</small></article>
          <article class="hero-card"><span>今日 Token</span><div class="token-pair" aria-label="今日 Token 输入输出"><p><span>输入</span><strong>{{ formatTokens(stats.today_input_tokens) }}</strong></p><p><span>输出</span><strong>{{ formatTokens(stats.today_output_tokens) }}</strong></p></div><small>今日消费 ${{ formatCost(stats.today_actual_cost) }}</small></article>
        </section>
