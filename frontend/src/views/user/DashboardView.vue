    ...
    <div class="dashboard-page">
      ...
      <template v-else-if="stats">
        ...
        <section class="hero-metrics">
          ...
          <article class="hero-card hero-card--with-sparkline"><span>今日 API 调用</span><strong>{{ formatNumber(stats.today_requests) }}</strong><small>全部请求 {{ formatNumber(stats.total_requests) }} 次</small><svg v-if="hasTrendData" class="metric-sparkline metric-sparkline--mint" viewBox="0 0 300 78" preserveAspectRatio="none"><path :d="requestSparklinePath" /></svg></article>
          <article class="hero-card hero-card--with-sparkline"><span>今日 Token</span><div class="token-pair" aria-label="今日 Token 输入输出"><p><span>输入</span><strong>{{ formatTokens(stats.today_input_tokens) }}</strong></p><p><span>输出</span><strong>{{ formatTokens(stats.today_output_tokens) }}</strong></p></div><small>今日消费 ${{ formatCost(stats.today_actual_cost) }}</small><svg v-if="hasTrendData" class="metric-sparkline metric-sparkline--violet" viewBox="0 0 300 78" preserveAspectRatio="none"><path :d="tokenSparklinePath" /></svg></article>
        </section>
        ...
        <section class="dashboard-grid">
          <article class="dashboard-card trend-card"><div class="card-heading"><div><h2>Token 使用趋势</h2><p>按选定时间范围汇总的个人 Token 用量</p></div></div><TokenUsageTrend :trend-data="trendData" :loading="loadingCharts" embedded /></article>
...
const timeRange = ref<DashboardTimeRange>('7d')
...
.dashboard-page .refresh-button { background:#fff; border-color:var(--line); box-shadow:0 4px 12px rgba(34,56,104,.04); }
...
.dark .dashboard-page .hero-card--primary {
  border-color: rgba(124, 185, 255, 0.72);
  background: linear-gradient(120deg, #255fc8 0%, #2c82d2 52%, #277c94 100%);
  box-shadow:
    0 18px 40px rgba(17, 76, 164, 0.32),
    inset 0 1px rgba(255, 255, 255, 0.26);
}

.dark .dashboard-page .hero-card--primary strong {
  color: #ffffff;
  text-shadow: 0 2px 12px rgba(3, 19, 48, 0.3);
}
...
.dark .dashboard-page .hero-card--primary > span,
.dark .dashboard-page .hero-card--primary small {
  color: #f5f9ff;
}
