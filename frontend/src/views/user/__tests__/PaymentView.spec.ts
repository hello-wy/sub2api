...
describe('PaymentView balance loyalty discount', () => {
  ...
  it('discounts the actual payment while submitting the original recharge amount', async () => {
    ...
    expect(text).not.toContain('payment.discountedPaymentAmount')
    expect(text).toContain('payment.actualPay')
    expect(text).toContain(formatPaymentAmount(100, 'CNY'))
    expect(text).not.toContain(formatPaymentAmount(92, 'CNY'))
...
describe('PaymentView subscription feature flag', () => {
  ...
  it('shows subscription plans when subscription_enabled is absent (opt-out default)', async () => {
  ...
  it('hides subscription plans while keeping recharge available when subscriptions are disabled', async () => {
  ...
  it('shows an unavailable notice instead of a doomed top-up form when balance recharge is disabled too', async () => {
    ...
    expect(wrapper.findComponent({ name: 'RechargePackageSelector' }).exists()).toBe(false)
    expect(wrapper.text()).toContain('payment.billingUnavailable')
    wrapper.unmount()
  })

  it('removes subscription plans when the flag flips off after mount', async () => {
    const wrapper = await mountSubscriptionPlanList(2)
    expect(wrapper.findAllComponents(SubscriptionPlanCard)).toHaveLength(2)
