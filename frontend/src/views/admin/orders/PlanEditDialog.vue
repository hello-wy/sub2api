  ...
  <BaseDialog :show="show" :title="plan ? t('payment.admin.editPlan') : t('payment.admin.createPlan')" width="wide" @close="emit('close')">
    <form id="plan-form" @submit.prevent="handleSavePlan" class="space-y-4">
      ...
      <div v-if="selectedGroupInfo" class="rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-dark-600 dark:bg-dark-800">
        ...
        <div v-if="selectedGroupInfo.subscription_quota_reset_mode === 'until_subscription_expires'" class="grid grid-cols-2 gap-2 text-xs">
          <div><span class="text-gray-500">{{ t('payment.admin.totalLimit') }}:</span> <span class="ml-1 font-medium text-gray-700 dark:text-gray-300">{{ selectedGroupInfo.subscription_total_limit_usd != null ? '$' + selectedGroupInfo.subscription_total_limit_usd : t('payment.admin.unlimited') }}</span></div>
        </div>
