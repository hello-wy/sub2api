...
export interface TestSchedule {
  enabled: boolean; interval_minutes: number; account_ids: number[]
  next_run_at?: string | null; last_run_at?: string | null; last_error?: string
}
...
export const intelligentTestsAPI = {
  ...
  async deleteRecord(id: number) { return (await apiClient.delete<{ deleted: number }>(`${base}/records/${id}`)).data },
  async deleteRecords(ids: number[]) { return (await apiClient.post<{ deleted: number }>(`${base}/records/delete-batch`, { ids })).data },
  async run(account_ids: number[], test_types: string[], idempotency_key: string, models?: Record<string, string>, reasoning_efforts?: Record<string, string>) {
    return (await apiClient.post<TestSubmission>(`${base}/run`, { account_ids, test_types, idempotency_key, ...(models ? { models } : {}), ...(reasoning_efforts ? { reasoning_efforts } : {}) })).data
  },
  ...
  async saveSetting(setting: TestSetting) {
    const snapshot: TestSetting = { ...setting, config: { ...setting.config }, ...(setting.schedule ? { schedule: { ...setting.schedule, account_ids: [...setting.schedule.account_ids] } } : {}) }
    const schedule = snapshot.schedule
    const { data } = await apiClient.put<{ updated: boolean; setting?: TestSetting }>(`${base}/settings/${encodeURIComponent(snapshot.test_type)}`, {
      enabled: snapshot.enabled, user_visible: snapshot.user_visible, config: snapshot.config,
      ...(schedule ? { schedule: { enabled: schedule.enabled, interval_minutes: schedule.interval_minutes, account_ids: schedule.account_ids } } : {})
    })
    return data.setting ?? snapshot
  },
