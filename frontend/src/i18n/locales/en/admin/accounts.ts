...
stateTicket: {
        ...
        list: { configure: 'Configure STATE', acquired: 'Acquired {length}', target: 'Target {length}', unknown: 'Refresh for status', expired: 'Expired; awaiting renewal', cooldown: 'Cooling down', renewing: 'Renewing', collapse: 'Collapse', more: 'View all {count} IPs' },
        quickStart: 'Enable and acquire',
        quickStartSaved: 'Settings were saved, but acquisition status could not be confirmed.',
        batch: {
          title: 'STATE quick setup',
          selectHint: 'Select accounts to configure STATE for their fixed IP channels',
          scope: '{count} accounts selected. This applies to all their non-retired IP channels.',
          fixedIPHint: 'Each fixed IP channel keeps its own ticket. Business requests continue using each channel’s fixed IP.',
          limit: 'Select at most 100 accounts and 500 expanded IP channels per operation.',
          action: 'Action', enable: 'Enable STATE', disable: 'Disable STATE',
          harvest: 'Acquire tickets immediately after saving',
          prerequisites: 'Configure the global acquisition proxy pool and enable its master switch, then refresh here.',
          harvestHint: 'Acquisition runs in the background within concurrency limits. Enabled channels without a valid ticket cannot yet route the target model. Unsupported accounts are skipped.',
          enableButton: 'Apply to selected accounts', disableButton: 'Disable STATE for selected accounts',
          summary: '{total} channels: {saved} saved, {skipped} skipped, {failed} failed.',
          resultHint: 'Submission results are a snapshot of this operation, not proof of a verified ticket. Refresh an individual channel to read its latest ticket status.',
          channel: 'Account / IP channel', result: 'Result at submission', status: 'Ticket status',
          failed: 'The operation or response was interrupted. Some accounts may already be saved; refresh the account list before retrying.',
          outcomes: { saved: 'Settings saved', harvesting: 'Acquisition started', ready: 'Valid ticket available', waiting: 'Saved; waiting', cooldown: 'Saved; cooling down', skipped: 'Skipped', error: 'Operation failed' }
        },
"experiment": "292 / 332 are experimental STATE string lengths; 312 is an experimental anomaly signal, not an HTTP status code. This feature cannot guarantee model quality or prevent upstream overload.",
"verifiedAt": "Business verification: {time}",
"verifiedModel": "Returned model: {model}",
"verificationHint": "Records the latest completed response using current credentials and protection on the fixed business IP. Local cache lifetime is not an upstream guarantee of future requests or model quality.",
"authenticationBlocked": "Authentication failed; repair credentials first",
"needsVerification": "Credentials or identity changed; verification required",
"modelLabel": "Target model",
"fixedProxyMissing": "Save this account / IP channel’s fixed proxy first.",
"quarantineHint": "A refreshed ticket does not clear existing model mismatch quarantine or manual pauses. Scheduling recovery still requires administrator confirmation.",
"channelsHint": "This account has multiple fixed IPs. Select it in the account list for STATE quick setup, or configure channels individually in Manage IPs. Settings here only affect the current channel.",
"channelTitle": "STATE settings · {name}",
"channelEntry": "STATE settings for this IP",
"discardTitle": "Discard unsaved STATE settings?",
"discardMessage": "This IP channel has unsaved STATE settings. Saved settings and tickets remain unchanged.",
  "title": "STATE ticket",
  ...
  "proxyHint": "Used to acquire STATE. The fixed business IP verifies current credentials and protection settings before use. Local caching lasts at most one hour, with verification before renewal.",
  ...
  "ready": "Business verification passed; cache remaining {time}",
  "savedTicket": "Saved ticket",
  "capturedAt": "Captured {time}",
  "expiresAt": "Local cache deadline {time}",
  ...
  "acquire": "Acquire now",
  "reacquire": "Reacquire",
  "failureHint": "Without a valid STATE, only this physical account / IP channel’s target model is gated. Disabling restores the normal ticket flow without clearing other pauses or quarantine.",
  ...
  "retry": "Retry",
  "states": {
    "authentication_blocked": "Authentication failed; repair credentials first",
    "ready": "Ticket available",
    "disabled": "Disabled",
    ...
    "error": "Acquisition failed"
      ...
      ipChannels: {
        order: 'IP order',
        orderHint: 'Balance total in-flight requests across accounts, then fill each account by IP order and channel ID. Earlier free IPs are reused before later ones. New IPs default to 50 concurrent requests, editable independently. Unavailable IPs are skipped; continuations requiring upstream state stay pinned. A limit of 0 is unlimited.',
        recover: 'Recover this IP', recoverAll: 'Recover all IP channels', recoverHint: 'Clear errors and cooldowns while preserving manual pauses',
        detailsUnavailable: 'IP channel details are temporarily unavailable. Close this editor, refresh the account list, and reopen it.',
        zeroUnlimited: '0 means no fixed limit',
        test: 'Test this IP', schedule: 'Scheduled tests for this IP',
        aggregate: { normal: 'All channels available', partial: 'Some channels available', unavailable: 'No available channels' },
        title: "IP channels · {name}",
        description: "Fixed IP channels for this account participate independently in request scheduling, with separate concurrency, priority and pause settings.",
        manage: "Manage IPs",
        requiresFixedProxy: "Set a fixed proxy first, or import this account again with multiple IPs selected.",
        manageCount: "View all {count} IPs",
        direct: "Direct connection",
        load: "In flight {current} / limit {max}",
        paused: "Paused",
        enabled: "Available",
        unavailable: "Unavailable",
        cooldown: "Cooling down until {until}",
        removeHint: "Before changing or removing an IP, pause the channel for at least 2 minutes and wait for all requests to finish. Keep at least one channel.",
        add: "Add fixed IP channels",
        perIPConcurrency: "Concurrency per IP",
        addSelected: "Add {count} IPs",
        removeTitle: "Remove IP channel",
        removeConfirm: "Remove {name}? Historical usage remains in account statistics.",
        failed: "Operation failed. Refresh and try again.",
        saved: "IP channels updated",
        pause: "Pause",
        resume: "Resume",
        editHint: "This account uses independent IP channels. Use Manage IPs in the account list to configure each channel’s proxy, concurrency, priority and traffic controls.",
      ...
      codexGateway: {
        label: "Codex request gateway",
        official: "Official endpoint (default)",
        useNew: "Use new gateway",
        searchPlaceholder: "Select a saved gateway or enter its full base URL",
        hint: "Saving the account remembers this gateway for other accounts. Only Codex requests use it; authorization and token refresh stay on official endpoints.",
        credentialsHint: "This gateway receives the account access token and request content. Only use gateways you trust.",
        inherited: "Inherits the parent account’s Codex gateway. Edit it on the parent account.",
        retry: "Could not load saved gateways. Click to retry, or enter a new URL.",
      },
      ...
      status: {
        ...
        upstream429ObservationHint: 'Upstream 429 is cooling down this channel.'
      },
      columns: {
        name: 'Name',
        id: 'Account ID',
      ...
      riskControl: {
        checkAction: 'Risk Check',
        suspected: 'Suspected Risk',
        abnormal: 'Risk Anomaly',
        normal: 'Normal',
        missing: 'No State',
        unchecked: 'Not Checked',
        uncheckedTitle: 'Risk check has not been run',
        badgeTitle: 'Upstream state length {length}, checked at {time}; length 312 indicates suspected risk',
        abnormalBadgeTitle: 'Upstream state length {length}, checked at {time}; expected length 292 or 332',
        detailTitle: 'Upstream state length {length}, checked at {time}',
        suspectedResult: 'Suspected risk detected: state length {length}',
        normalResult: 'Risk check passed: state length {length}',
        abnormalResult: 'Unexpected state length: {length} (HTTP {status})',
        missingResult: 'Upstream returned no state (HTTP {status})',
        checkFailed: 'Risk check failed'
      },
      ...
      openai: {
        ...
        testMode: 'Test mode',
        diagnostics: {
          mode: 'Gateway acceptance (HTTP / WebSocket / compact)',
          title: 'Actual destination and capabilities',
          hint: 'Tests three capabilities using requests that may incur usage, with a 45-second limit each. WebSocket tests native upstream support without changing forwarding settings. Addresses are this service’s actual targets, not the gateway’s internal destinations.',
          source: 'Configuration source',
          target: 'Final target URL',
          noRequest: 'No request sent yet',
          cancel: 'Cancel test',
          failed: 'Some capabilities did not pass. See individual results.',
          states: {
            pending: 'Pending',
            running: 'Running',
            passed: 'Passed',
          ...
          errors: {
            configuration_error: 'Unable to resolve credentials or gateway configuration.',
            plugin_transport: 'A plugin owns the transport and its internal destination is not observable. Native probes were skipped.',
            authentication_error: 'Unable to obtain valid authentication.',
            probe_failed: 'The request did not complete a valid response.',
            http_error: 'Upstream returned an HTTP error. Check the status code.',
            timeout: 'The probe exceeded 45 seconds and was stopped.',
            cancelled: 'Test cancelled.',
            websocket_handshake_failed: 'WebSocket handshake failed.',
            incomplete_response: 'Connection ended without a complete test result.',
            response_failed: 'Upstream returned a failed or incomplete response.',
            compact_missing: 'No compaction output was returned; native v2 compaction did not pass.',
          },
          compactLabel: 'compact (native v2)',
        },
