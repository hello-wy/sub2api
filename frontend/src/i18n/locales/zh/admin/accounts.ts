...
stateTicket: {
        defaults: 'STATE 默认值',
        list: { configure: '配置 STATE', acquired: '已取得 {length}', target: '目标 {length}', unknown: '状态待刷新', expired: '已到期，等待刷新', cooldown: '冷却中', renewing: '续期中', collapse: '收起', more: '查看全部 {count} 个 IP' },
        quickStart: '一键启用并获取',
        quickStartSaved: '设置已保存，但未能确认采集状态。',
        batch: {
          title: 'STATE 一键配置',
          selectHint: '勾选账号后，为它们的固定 IP 通道批量配置 STATE',
          scope: '已选择 {count} 个账号，操作会覆盖这些账号的全部未移除 IP 通道。',
          fixedIPHint: '每个固定 IP 通道分别保存票据，日常请求继续使用各自的固定 IP。',
          limit: '每次最多选择 100 个账号，展开后最多 500 个 IP 通道。请缩小选择范围。',
          action: '操作', enable: '开启 STATE', disable: '关闭 STATE',
          harvest: '保存后立即获取票据',
          prerequisites: '开启前请配置全局采集 IP 池并打开总开关；保存后回到这里刷新。',
          harvestHint: '采集按并发限制在后台执行。开启后，尚无有效票据的通道暂不能调度目标模型；不支持的账号会跳过。',
          enableButton: '一键应用', disableButton: '一键关闭所选账号的 STATE',
          summary: '共 {total} 条通道：设置已保存 {saved}，跳过 {skipped}，失败 {failed}。',
          resultHint: '下方提交结果是本次操作的快照，不代表票据已经验证成功。点击刷新可查看某条通道的最新票据状态。',
          channel: '账号 / IP 通道', result: '提交时结果', status: '票据状态',
          failed: '操作未完成或响应丢失，部分账号可能已保存。请刷新列表确认后再重试。',
          outcomes: { saved: '设置已保存', harvesting: '采集已启动', ready: '已有可用票据', waiting: '已保存，等待采集', cooldown: '已保存，冷却中', skipped: '已跳过', error: '操作失败' }
        },
"experiment": "292 / 332 是 STATE 字符串的实验筛选长度；312 是实验异常信号，不是 HTTP 状态码。该功能不保证模型质量或消除上游过载。",
"verifiedAt": "业务复验：{time}",
"verifiedModel": "返回模型：{model}",
"verificationHint": "记录当前凭据与保护策略下固定业务 IP 的最近一次完整响应。缓存期限不代表上游承诺，也不保证后续请求或模型质量。",
"authenticationBlocked": "认证失败，先修复登录凭据",
"needsVerification": "凭据或身份已变化，等待重新验证",
"modelLabel": "目标模型",
"fixedProxyMissing": "请先保存此账号 / IP 通道的固定代理。",
"quarantineHint": "票据重新获取成功不会自动解除已有的“降智”隔离或手动暂停；恢复调度仍需管理员确认。",
"channelsHint": "此账号使用多个固定 IP。可在账号列表勾选后使用“STATE 一键配置”，或在“管理 IP”中逐通道配置；这里只修改当前通道。",
"channelTitle": "STATE 设置 · {name}",
"channelEntry": "此 IP 的 STATE 设置",
"discardTitle": "放弃未保存的 STATE 设置？",
"discardMessage": "当前 IP 通道的 STATE 设置尚未保存。已保存的设置和票据不受影响。",
  "title": "STATE 票据",
  ...
  "proxyHint": "仅用于获取 STATE。固定业务 IP 使用当前凭据和保护策略复验后才可使用；本地缓存最多保留一小时，并提前自动复验。",
  "sidHint": "支持 HTTP / SOCKS5h 代理。动态代理用户名中的 {'{'}sid{'}'} 会在每次尝试时自动替换为新的会话标识。",
  "model": "目标模型：{model}",
  "ready": "业务复验通过，缓存剩余 {time}",
  "savedTicket": "已保存票据",
  "capturedAt": "获取时间：{time}",
  "expiresAt": "本地缓存截止：{time}",
  ...
  "acquire": "立即获取",
  "reacquire": "重新获取",
  "failureHint": "启用后若没有有效 STATE，仅限制此物理账号 / IP 通道的目标模型调度。关闭并保存会恢复普通票据流程，但不会解除其他暂停或隔离。",
  "saved": "票据设置已保存。",
  ...
  "retry": "重试",
  "states": {
    "authentication_blocked": "认证失败，先修复登录凭据",
    "ready": "最近业务复验通过",
    "disabled": "未启用",
    ...
    "error": "获取失败"
      ...
      ipChannels: {
        order: 'IP 使用顺序',
        orderHint: '账号之间按总在途请求均衡分配；账号内部按 IP 顺序值从小到大、再按通道 ID 使用。前序 IP 占满才使用后序，有空位时优先补回。新 IP 默认并发 50，可独立调整；不可用通道跳过，需要上游状态的续写保持原连接。并发 0 表示不限额。',
        recover: '恢复此 IP 状态', recoverAll: '恢复所有 IP 状态', recoverHint: '清除错误和冷却，保留管理员手动暂停设置',
        detailsUnavailable: '账号的 IP 通道信息暂时无法读取，请关闭并刷新账号列表后重新打开。',
        zeroUnlimited: '0 表示不设固定上限',
        test: '测试此 IP', schedule: '此 IP 的定时测试',
        aggregate: { normal: '全部通道可用', partial: '部分通道可用', unavailable: '暂无可用通道' },
        title: "IP 通道 · {name}",
        description: "同一账号的多个固定 IP 独立参与请求调度。每条通道分别设置并发、优先级与启停状态。",
        manage: "管理 IP",
        requiresFixedProxy: "请先设置固定代理，或重新导入此账号并选择多个 IP。",
        manageCount: "查看全部 {count} 个 IP",
        direct: "服务器直连",
        load: "执行中 {current} / 并发上限 {max}",
        paused: "已暂停",
        enabled: "可用",
        unavailable: "不可用",
        cooldown: "冷却至 {until}",
        removeHint: "更换或移除 IP 前，请先暂停该通道至少 2 分钟，并确认执行中请求为 0。至少保留一条通道。",
        add: "添加固定 IP 通道",
        perIPConcurrency: "每个 IP 的并发",
        addSelected: "添加 {count} 个 IP",
        removeTitle: "移除 IP 通道",
        removeConfirm: "确认移除 {name}？历史用量仍保留在账号统计中。",
        failed: "操作失败，请刷新后重试",
        saved: "IP 通道已更新",
        pause: "暂停",
        resume: "恢复",
        editHint: "该账号使用独立 IP 通道。请在账号列表的“管理 IP”中设置各通道的代理、并发、优先级与流量控制。",
        stats: "查看通道统计",
        loadFactor: "调度权重",
        traffic: "流量控制",
      },
      ...
      codexGateway: {
        ...
        official: "官方直连（默认）",
        useNew: "使用新网关",
        searchPlaceholder: "选择已有网关，或输入完整基础地址",
        hint: "保存账号后会自动记住网关，供其他账号复用。仅影响 Codex 请求，授权和 Token 刷新仍使用官方接口。",
        credentialsHint: "该网关将接收此账号的访问令牌及请求内容，请仅使用可信的网关。",
        inherited: "继承母账号的 Codex 请求出口，请在母账号中修改。",
        retry: "历史网关加载失败，点击重试；仍可输入新地址。",
      },
      title: '账号管理',
      ...
      riskControl: {
        ...
        missing: '未返回',
        unchecked: '未检测',
        uncheckedTitle: '尚未执行风控检测',
        badgeTitle: '上游 state 长度 {length}，检测于 {time}；长度 312 判定为疑似风控',
        abnormalBadgeTitle: '上游 state 长度 {length}，检测于 {time}；长度不属于正常值 292/332',
        detailTitle: '上游 state 长度 {length}，检测于 {time}',
        suspectedResult: '检测到疑似风控：state 长度 {length}',
        normalResult: '风控检测正常：state 长度 {length}',
        abnormalResult: 'state 长度异常：{length}（HTTP {status}）',
        missingResult: '上游未返回 state（HTTP {status}）',
        checkFailed: '风控检测失败'
      },
      noAccountsYet: '暂无账号',
      ...
      status: {
        ...
        upstream429ObservationHint: '上游 429 冷却中，当前通道暂不参与调度。'
      },
      tempUnschedulable: {
        title: '临时不可调度',
      ...
      openai: {
        ...
        testMode: '测试模式',
        diagnostics: {
          mode: '出口与能力验收（HTTP / WebSocket / compact）',
          title: '实际出口与能力结果',
          hint: '将分别测试三项能力，探测请求可能产生用量。每项最多等待 45 秒；WebSocket 检测上游原生能力，不修改当前转发模式。地址为本服务实际请求目标，不代表网关内部转发地址。',
          source: '配置来源',
          target: '最终目标地址',
          noRequest: '尚未发送请求',
          cancel: '取消测试',
          failed: '部分能力未通过，请查看各项结果。',
          states: {
            pending: '等待测试',
            running: '测试中',
            passed: '通过',
          ...
          errors: {
            configuration_error: '无法解析账号凭据或出口配置。',
            plugin_transport: '插件接管了请求，无法观测其内部出口；本次未执行原生探测。',
            authentication_error: '无法取得有效的认证信息。',
            probe_failed: '请求未完成有效响应，当前能力未通过。',
            http_error: '上游返回 HTTP 错误，请检查状态码。',
            timeout: '测试超过 45 秒，已停止。',
            cancelled: '测试已取消。',
            websocket_handshake_failed: 'WebSocket 握手失败。',
            incomplete_response: '连接已结束，但未收到完整测试结果。',
            response_failed: '上游返回失败或未完成的响应。',
            compact_missing: '未返回 compaction 输出，原生 v2 压缩能力未通过。',
          },
          compactLabel: 'compact（原生 v2）',
        },
