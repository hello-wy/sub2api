export default {
    scheduledTests: {
      ...
      cancelGroup: '中断',
      cancelGroupHint: '停止本次请求和自动重试，保留后续定时计划',
      groupStatusCancelling: '正在中断',
      groupCancelRequested: '已请求中断，正在停止当前任务',
      groupCancelError: '中断测试失败',
      triggerGroup: '立即执行一次',
      groupStarted: '测试已启动，当前状态将自动更新',
      groupRetryHint: '失败结果会在 5 秒、15 秒后自动重试，最多重试 2 次；每次请求仍按正常规则计费。暂停或编辑计划会停止后续重试。',
      groupPhaseWaiting: '等待模型响应',
      groupPhaseReceiving: '正在接收流',
      groupPhaseThinking: '模型思考中',
      groupPhaseGenerating: '正在生成作品',
      groupPhaseSaving: '正在保存结果',
      groupStatusStarting: '正在启动',
      groupStatusRunning: '正在测试',
      groupStatusRetrying: '等待自动重试',
      groupStatusSuccess: '测试成功',
      groupStatusFailed: '测试失败',
      groupStatusInterrupted: '执行已中断',
      groupStatusReady: '等待定时执行',
      groupStatusPaused: '已暂停',
      groupProgress: '第 {attempt}/{max} 次尝试 · 已完成 {completed}/{total} · 成功 {succeeded} · 失败 {failed}',
      groupElapsed: '已用时 {seconds} 秒',
      groupRetryCountdown: '{seconds} 秒后重试失败结果',
      groupLastError: '最近错误：{message}',
      groupStatusReconnect: '状态更新暂时失败，正在重新连接…',
