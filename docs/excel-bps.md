...
本入口面向 HTTP `/v1/responses` 和 `/v1/responses/compact`。账号可指定使用 BPS 的模型，按账号映射后的模型名匹配；其他模型继续使用原有协议。界面首次开启默认选择 `gpt-6-astra`；历史账号仅有开关而没有模型列表时保留全模型行为，显式空列表则不启用任何模型。支持单账号编辑及批量修改。

实际选择 BPS 时强制上游 HTTP/SSE，优先于账号的自动透传和 WS mode。保持原始模型名或显式账号映射，不因模型权限不足偷偷切换模型。现有调度、分组授权和并发额度继续生效；开关不会重新启用已停用的账号。
...
`max`/`ultra` 映射 `xhigh`，`none`/`minimal` 映射 `low`，请求和实际 effort 的差异出现在用量明细中。支持 `json_object` 和 `json_schema` 结构化输出：通过提示词约束生成，完整响应经本地 JSON/Schema 校验后才转发，不支持外部 Schema 引用。仅提供 `previous_response_id` 而没有完整历史的请求仍不受支持。不要把 HTTP 200 当作模型能力证明。

需要原生图片生成、联网高强度搜索或指定相关托管工具的请求会显式绕过 BPS，响应头给出 `X-Codex2API-Basispoints-Bypass` 原因。其余不支持的工具约束或内容会返回明确错误。BPS 工具往返探测禁止这种原生回退。账号的普通文本测试遵循模型范围，原生压缩和图片专项测试仍保留各自的专用流程。

可选开启「遇到 403 自动关闭 Excel / BPS 协议」；遇到适用的 403 后仅关闭该协议，保持账号原有启停状态。429 响应同步用量快照及冷却时间，参与现有调度限流。
...
- 管理员可设置请求体上限 (1–128 MiB)、估算资源预算 (512–2048 MiB) 和每进程最大在途请求数 (1–512), 默认分别为 64 MiB、1024 MiB 和 128 个. 配置预算必须至少为请求体上限的 8 倍; 有效预算取配置值与「在途请求上限 × 8 MiB」中的较大值，因此 512 个槽位时至少为 4096 MiB. 已配置的更小网关上限仍生效. 已知长度按请求体大小的 8 倍预留, 最小按 1 MiB 计算; 未知长度的未压缩请求随读取量增加预留, 压缩请求在解压期间按配置的请求体上限预留. 正文读取完成后, 未知长度和压缩请求按实际大小保留预算至请求结束. 大请求可用并发低于配置上限.
...
本次移植的来源提交、兼容调整和验证记录见 [excel-bps-port.md](excel-bps-port.md)。本仓库验证使用本地测试和 mock 上游，不代表真实账号权限或生产部署已验收。
